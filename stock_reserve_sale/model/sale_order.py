...
class SaleOrder(models.Model):
    ...
    def _compute_reserves_count(self):
        reserve_data = self.env["stock.reservation"].read_group(
            domain=[("sale_id", "in", self.ids)],
            fields=["sale_id"],
            groupby=["sale_id"],
        )
        mapped_data = {m["sale_id"][0]: m["sale_id_count"] for m in reserve_data}
        for order in self:
            order.reserves_count = mapped_data.get(order.id, 0)
            lines = order.order_line.filtered(
                lambda li: li.product_id.detailed_type != "service"
            )
            order.all_lines_reserved = order.reserves_count == len(lines)
    ...
    def write(self, vals):
        ...
        for order in self:
            ...
            for line in vals.get("order_line", []):
                if line[0] == Command.UPDATE and list(
                    set(line[2].keys()).intersection(_LINE_KEYS)
                ):
